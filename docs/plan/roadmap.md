...
| M10 | 报表 / 仪表盘（含 VAT 申报） | 🟡 规划完成·可施工（`milestones/M10.md`；5 步：P/L → ⭐BTW 申报汇总 → ICP → 开支报表 → Dashboard）。**税法决策已于 2026-06-15 与作者对照官方指南 `docs/insight/btw-aangifte-2026-guide.md`（Opus 通读 41 页）逐条共定冻结**：NL ruleset 按 `company.country_code` 选+其它国 fallback+banner、hoog/laag/zero 税率档位落盘默认 21/9/0、5b 全额抵+私用走 1d（年末按 business% 算）、5a/5c 为辅助合计（官方只命名 5b）、EU 内采购=4b（非 art.23 进口）、非欧盟进口/境内反向征收/OSS/herziening/KOR 均 N/A v1、报表带免责声明 |
| M11 | 收尾 / GA 前体检 | ⬜ |
