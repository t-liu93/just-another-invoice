...
- [ ] **包名/端口**：Python 包 `jai`；后端/app 默认 `:8000`（可用 `APP_PORT` 覆盖），前端 dev `:5173`，Postgres 容器内 `:5432`；基础 compose 不暴露 DB 端口；本地开发用 `docker-compose.dev.yml` 绑定 `127.0.0.1:${POSTGRES_DEV_PORT:-5433}`，需要改端口时用 `--env-file .env --env-file .env.dev` 叠加 `.env.dev`；DB 名 `jai`。
...
- **端口**：后端/app 默认 `8000`（可用 `APP_PORT` 覆盖），前端 dev `5173`（Vite 把 `/api` 代理到 `http://localhost:${APP_PORT:-8000}`）。
- **环境变量**（`.env.example` 落地；Docker Compose 自动读取根目录 `.env`）：数据库连接通过独立的 ``POSTGRES_*`` 变量配置：``POSTGRES_HOST``（默认 ``localhost``）、``POSTGRES_PORT``（默认 ``5432``）、``POSTGRES_USER``（默认 ``jai``）、``POSTGRES_PASSWORD``（默认 ``jai``）、``POSTGRES_DB``（默认 ``jai``）。后端用 ``sqlalchemy.engine.URL.create()`` 自动拼装 ``database_url``，正确处理密码中的特殊字符。如需完全控制，可直接设 ``DATABASE_URL`` 覆盖。另有 ``APP_PORT``（后端/app 监听端口，默认 ``8000``）、``STATIC_DIR``（部署态指向前端 ``dist``）、``APP_VERSION``。开发态在 ``.env`` 里覆盖 ``POSTGRES_PORT=5433``（与 ``docker-compose.dev.yml`` 的 ``POSTGRES_DEV_PORT`` 对齐）；生产 Docker Compose 在 app service 里设 ``POSTGRES_HOST=postgres``（走内网），其余走默认。
...
- **独立验证**：`uv run uvicorn jai.main:app --reload --port "${APP_PORT:-8000}"` + `curl "localhost:${APP_PORT:-8000}/api/health"` 得 ok；httpx 测试客户端断言 200。
...
uv run uvicorn jai.main:app --port "${APP_PORT:-8000}"   # 另开终端：curl "localhost:${APP_PORT:-8000}/api/health" → {"status":"ok",...}
```
...
### B1 · 前端脚手架
- Vite + Vue 3 + TS：`package.json`（deps `vue`/`vue-router`/`pinia`/`naive-ui`/`echarts`/`vue-i18n`；dev `typescript`/`vue-tsc`/`vite`/`@vitejs/plugin-vue`/`openapi-typescript`）；`.npmrc`（`legacy-peer-deps=true`）；`tsconfig*.json`；`vite.config.ts`（`server.proxy['/api'] → http://localhost:${APP_PORT:-8000}`）；`index.html`；`src/main.ts`；`src/App.vue`；`src/styles/`。`npm run build = vue-tsc -b && vite build`。
...
- `src/api/`：轻量 fetch 封装；`package.json` 加 `"codegen": "node scripts/codegen.mjs"`，脚本从 `OPENAPI_URL`、`APP_PORT` 或根 `.env` 推导 `http://localhost:${APP_PORT:-8000}/api/v1/openapi.json`，并在生成前校验 OpenAPI title。**Track A 已先完成**：确认已有本地 `.env`（首次 checkout：`cp .env.example .env`），起一份后端（`docker compose -f docker-compose.yml -f docker-compose.dev.yml up -d postgres` + `uv run uvicorn ... --port "${APP_PORT:-8000}"`），跑 `npm run codegen` 生成并**提交真实的 `schema.d.ts`**（不要手写桩，避免与后端漂移）。
...
- compose 加 `app` service：`build .`、`depends_on postgres`、env `POSTGRES_HOST=postgres`（或显式 `DATABASE_URL` 指向 compose 内 postgres）、端口映射 `${APP_PORT:-8000}:${APP_PORT:-8000}`、data 卷。
- **验证**：`docker compose up --build` → app + postgres 起来。

### C2 · 后端托管 SPA（端到端打通）
- 确认 `main.py` 在 `STATIC_DIR` 托管 `dist` + 非 `/api` 路由 SPA fallback。
- **验证**：`docker compose up` → 浏览器 `:${APP_PORT:-8000}`（注意是 app 端口，非 5173）看到占位页 + `/api/health` ok。
...
2. 浏览器开 `http://localhost:${APP_PORT:-8000}` → 看到（空）占位页，能切 EN/ZH，（可选）显示 "backend: ok"。
3. `curl "http://localhost:${APP_PORT:-8000}/api/health"` → `{"status":"ok",...}`。
