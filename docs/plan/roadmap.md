...
- **关键决策（作者已定 · 详见 `milestones/M7.5.md`）**：方法 **B**（行可 3 位、汇总到文档级再到分）；适用全部文档级出参（发票/报价/估算对客面/开支）；**不改列类型**（`NUMERIC(18,3)` 容得下 2 位）；**不舍到整欧元**（VAT 须按分申报）；引入 `currency_minor_unit`（EUR=2 硬编码 + 口子）；**历史 3 位发票重算落库**。
...
| M7.5 | 货币舍入口径修正（落到「分」） | 🟡 设计完成（2026-06-13；`milestones/M7.5.md` 决策 D1–D7 与作者共定，方法 B；M7 walkthrough 暴露 `F2026-009` 不可收满引出，排在 M8 前，未开工） |
| M8 | 开支（含 AI 填单 + AI 供货价单识别，可与单据线并行） | ⬜（`milestones/M8.md` 骨架/预研已落，未冻结，进入时 JIT 补全） |
