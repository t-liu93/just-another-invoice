...
- [ ] **Postgres 版本**：默认 `postgres:16`。
- [ ] **包名/端口**：Python 包 `jai`；后端 `:8000`，前端 dev `:5173`，Postgres `:5432`；DB 名 `jai`。
...
- **环境变量**（`.env.example` 落地）：`DATABASE_URL=postgresql+asyncpg://jai:jai@localhost:5432/jai`、`STATIC_DIR`（部署态指向前端 `dist`）、`APP_VERSION`。
...
- **0.1** 建目录骨架：`backend/`（空 `src/jai/__init__.py`）、`frontend/`（占位）；根 `.gitignore`（Python/Node/.env/data）、`.env.example`（见上契约）、`README.md` 桩、`docker-compose.yml` **只含 `postgres` service**（`postgres:16`，env `POSTGRES_USER/PASSWORD/DB=jai`，端口 `5432:5432`，命名卷）。
- **独立验证**：`docker compose up -d postgres` 起得来；`psql` 能连上。
...
- `src/api/`：轻量 fetch 封装；`package.json` 加 `"codegen": "openapi-typescript http://localhost:8000/api/v1/openapi.json -o src/api/schema.d.ts"`。**Track A 已先完成**：起一份后端（`docker compose up postgres` + `uv run uvicorn`），跑 `npm run codegen` 生成并**提交真实的 `schema.d.ts`**（不要手写桩，避免与后端漂移）。
...
4. 开发态另测：`docker compose up postgres` + 后端 `uv run uvicorn ... --reload` + 前端 `npm run dev`，`:5173` 占位页正常、`/api` 代理通。
5. CI 四关全绿。
