...
| M2.5 | 设置入口统一 + 设置 UX 重构（Affine 式可展开面板） | 🟢 完成（2026-06-08；面板收敛 + 自测点 1–6 + SMTP radio follow-up 全过）｜收尾 follow-up：界面语言账号级持久化（见 M2.5.md 步骤 4） |
| M3 | 客户 | ⬜ |
| M4 | 字典 / 主数据（+ 产品/材料目录） | ⬜ |
| M5 | 定价引擎 + 发票核心 | ⬜ |
| M6 | 报价 + 转换 + 内容模板 | ⬜ |
| M6.5 | 成本核算 / 报价辅助（内部 estimation → 报价） | ⬜ |
| M7 | 收款 | ⬜ |
| M8 | 开支（含 AI 填单 + AI 供货价单识别，可与单据线并行） | ⬜ |
| M9 | PDF（邮件底座在 M1） | ⬜ |
| M10 | 报表 / 仪表盘（含 VAT 申报） | ⬜ |
| M11 | 收尾 / GA 前体检 | ⬜ |
