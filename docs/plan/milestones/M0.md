...
- `src/api/`：轻量 fetch 封装；`package.json` 加 `"codegen": "openapi-typescript http://localhost:8000/api/v1/openapi.json -o src/api/schema.d.ts"`。**Track A 已先完成**：确认已有本地 `.env`（首次 checkout：`cp .env.example .env`），起一份后端（`docker compose -f docker-compose.yml -f docker-compose.dev.yml up -d postgres` + `uv run uvicorn`），跑 `npm run codegen` 生成并**提交真实的 `schema.d.ts`**（不要手写桩，避免与后端漂移）。
...
- compose 加 `app` service：`build .`、`depends_on postgres`、env `POSTGRES_HOST=postgres`（或显式 `DATABASE_URL` 指向 compose 内 postgres）、`8000:8000`、data 卷。
