...
> 进入本里程碑前 JIT 产出。先读 `docs/plan/roadmap.md` §2 全局约束（**尤其红线 1 算钱在 services / 红线 12 VAT 数据驱动**）+ M10 那一格；上游 `docs/insight/InvoiceShelf-analysis.md` **§7.4 荷兰 VAT(BTW) 模型与申报**（权威）、§7.1F 报表清单；以及落盘字段来源 `milestones/M5.md`（发票 VAT 快照）、`milestones/M8.md`+`M8.5.md`（开支 VAT / `business_percentage` / `depreciation_years`）。
>
...
- **对应文档**：分析文档 §7.4（全节）/ §7.1F；roadmap M10、§2 红线 1/6/12；`M4.md`（report_box 留空的约定）、`M5.md`（发票 VAT 快照列）、`M8.5.md`（business%/折旧顺延本里程碑）。
...
>
> **初稿提案（基于分析文档 §7.4.2/§7.4.7/§7.4.8，🔴 待作者对照上述指南校对）：**
