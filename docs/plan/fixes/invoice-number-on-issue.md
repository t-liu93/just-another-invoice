...
- **Acceptance (self-test points passed?)**: **manual walkthrough performed 2026-06-23 — all 🟢 Deployment Self-Test Points passed** (Concept / 草稿 display in list / editor / preview PDF; delete-draft-leaves-no-gap; quote→invoice deferral; email / payment issue-gating). Automated coverage backs each point through real HTTP endpoints. One regression surfaced during the walkthrough and was fixed in `0520c48`: the invoice payments aggregate endpoint (`GET /invoices/{id}/payments`, called on every invoice view including drafts) returned HTTP 500 for an unnumbered draft — it now tolerates a draft (null `invoice_number`, empty `items`). A separate symptom — a normal refresh on `localhost:8000` loading a different app / bouncing to login — was diagnosed as a stale Service Worker registered by another project on the same dev origin (**not a jai bug**: jai registers no SW and serves `index.html` with `no-cache`); resolved by unregistering the SW / clearing the browser's site data.
