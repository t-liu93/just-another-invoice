...
>
> （`CLAUDE.md` 是指向本文件 `AGENTS.md` 的软链接；中文镜像是 `AGENTS_zh.md` —— 见「文档语言」。）
...
## 文档语言（English-first + 中文镜像）
- **所有 check-in 的文档一律英文优先**：权威文件 `X.md` 为英文。适用于 `docs/` 下全部文件、`README`、以及本 agent 文件（`AGENTS.md` / `AGENTS_zh.md`）。
- **每个文档都配一份中文镜像 `X_zh.md`**，与英文**语义完全一致**——它是给作者阅读用的。两个版本必须同步：**改一个就在同一改动里改另一个**，绝不允许漂移。
- **命名规则**：英文 = `X.md`，中文 = `X_zh.md`（如 `readme.md` / `readme_zh.md`、`roadmap.md` / `roadmap_zh.md`）。英文版保留干净的权威名。
- **交叉引用**：每对在 H1 正下方加一行 blockquote 互链：
  - `X.md`（英文）：`> 🌐 **English** · [中文](X_zh.md)`
  - `X_zh.md`（中文）：`> 🌐 [English](X.md) · **中文**`
- **同语言互链**：`X.md` 里链到别文档的 `.md`，`X_zh.md` 里链到它们的 `_zh.md`。唯一例外是 `🌐` 交叉引用头本身——它是语言切换，故意指向另一语言的文件。
...
## 维护本文件
只在**根基**变化时才改本文件：技术栈、上面这些红线/命令/约定、执行模型（Agent orchestration）、或新增一个 agent 工具。**里程碑的推进不需要动它**——那只更新 `docs/plan/`。**本文件是双语的（English-first + 中文镜像）：任何改动都必须在同一次改动里同步更新 `AGENTS.md`，保持两版语义完全一致（见「文档语言」）。**
