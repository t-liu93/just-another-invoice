"""Integration tests for POST /api/v1/invoices/calculate (M5 step 1).
...
async def _setup_company(client: AsyncClient) -> dict:
    ...
    # Get seeded VAT rates and treatments
...
async def _create_customer(
    ...
    *,
    ...
    """Create a customer with a billing address and return the ID."""
    resp = await client.post(
        "/api/v1/customers",
        json={
            "name": name,
...
def _calc_payload(
    ...
    *,
    ...
    """Build a calculate request payload."""
    if lines is None:
        lines = [
            {
                "name": "Service",
...
@pytest.mark.integration
class TestVatTreatmentDerivation:
    ...
    async def test_customer_no_billing_country_derives_nl_domestic(
        ...
        await _full_auth(db_client)
        seeds = await _setup_company(db_client)
        # Create customer without billing address
        resp = await db_client.post(
            "/api/v1/customers",
...
class TestCalculateValidation:
    ...
    async def test_non_base_currency_422(self, db_client: AsyncClient) -> None:
        """M5: submitting USD when base is EUR → 422."""
        await _full_auth(db_client)
        seeds = await _setup_company(db_client)
        customer_id = await _create_customer(db_client)
        rate_21 = seeds["rates"]["NL standard (21%)"]["id"]
    ...
    async def test_cross_company_customer_422(self, db_client: AsyncClient) -> None:
        """Customer from another company → derivation fails → 422."""
        await _full_auth(db_client)
        seeds = await _setup_company(db_client)
        rate_21 = seeds["rates"]["NL standard (21%)"]["id"]

        # Use a random UUID that doesn't belong to any customer in this company
    ...
    async def test_inactive_treatment_422(self, db_client: AsyncClient) -> None:
        ...
        # Create and deactivate a treatment
    ...
    async def test_purchase_treatment_rejected_422(self, db_client: AsyncClient) -> None:
        ...
        resp = await db_client.post(
            ...
            json=_calc_payload(
                customer_id,
                rate_21,
                treatment_id=purchase_treatment_id,
            ),
        )
        assert resp.status_code == 422
