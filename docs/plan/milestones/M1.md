...
## 目标与范围
- **目标**：一个**能注册→登录（强制 MFA）→进到空 dashboard** 的私有应用；并把**邮件底座 + 三层设置表**一并落地（密码重置邮件能发出去、SMTP 在设置页可配）。
- **纳入（IN）**：
  - **设置表底座**：单表 `setting`（key-value + `level` + `scope_id` + JSONB `value`），类型化访问 + 内存缓存 + `user→company→global` 回退（M1 只用 GLOBAL 层）。
...
- **不纳入（OUT / 留到后续）**：
  - 公司业务档案（抬头/logo/VAT/本位币/编号规则）→ **M2**；设置表的 **company/user 层语义补全 + 缓存调优** 也在 M2。
...
- **`setting`**（设置单表，红线 5/6 的基础）：
  ...
  - **唯一约束** `(level, scope_id, key)`。M1 只写 GLOBAL 层（`onboarding.completed`、`smtp.*`）。`scope_id` 的 FK 留到 M2 公司/用户表落地再加。
...
- **后端**：
  ...
  - `services/settings.py`：`get_setting(key, level, scope_id) -> typed` / `set_setting(...)`；**`user→company→global` 回退**；**进程内缓存**（写时失效）。值经 Pydantic 解析，**杜绝 `'YES'/'NO'`**（红线 5）。
  - Alembic：建 `setting` 表（M1 首迁移）。
- **前端**：无（仅 codegen 不变）。
- **测试**：pytest——回退优先级、缓存命中/失效、类型化解析（bool/int/嵌套）、唯一约束冲突。
- **DoD**：见 roadmap §5。
