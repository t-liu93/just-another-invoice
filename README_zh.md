...
面向自由职业者和小微企业的自托管发票与业务管理系统。发票、报价、开支、收款、PDF + 邮件，以及荷兰 **BTW（增值税）申报**汇总——全部跑在单个 Docker 容器里。

技术栈 **FastAPI + Vue 3 + PostgreSQL**。

> **状态——早期 / 预发布（`v0.x`）。** 开源、自托管。**目前还没有发布 Docker 镜像**，所以下面的 Quick Start 是从源码自行 build 的版本。等镜像发布后会再补一份「拉取即运行」的 Quick Start。
...
- **平台** —— TOTP 两步验证、类型化三层设置、`Decimal` 金额计算、双语界面（English / 中文）、单容器部署。
...
| 打包 | 单 Docker 容器（前端构建 + 后端 + uvicorn）+ PostgreSQL |

## Quick Start（从源码构建）

发布镜像暂时还没有，所以本地自行 build 镜像，再用生产单容器 compose 启动。
...
里程碑 M0–M10 已完成（认证、设置、客户、目录、发票、报价、成本核算、收款、开支、PDF/邮件、报表 + 荷兰 BTW 申报）。M11（自托管收尾：备份、i18n 补全、文档）待做。详见 [`docs/plan/roadmap_zh.md`](docs/plan/roadmap_zh.md)。
